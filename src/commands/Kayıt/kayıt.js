    ...
    collector.on("collect", async (button) => {
      ...
      if(button.customId === "erkek") {
        ...
        const guild = message.guild
            ...
            const kemosalvo = client.users.cache.get(config.bot.owner);
        const bgerkekemb = new MessageEmbed().setColor(message.member.displayHexColor).setAuthor({name: message.member.displayName, iconURL: author.avatarURL({ dynamic: true, size: 2048 })}).setFooter({text: (config.bot.BotFooter) , iconURL: kemosalvo.avatarURL({ dynamic: true, size: 2048 })})
        if (!names) {
...
msg.edit({
  ...
  if(button.customId === "kadin") {
    ...
    const guild = message.guild
        ...
        await guild.members.cache.get(member.id).roles.remove(config.registration.unregistered)
        const kemosalvo = client.users.cache.get(config.bot.owner);
    const bgkadinemb = new MessageEmbed().setColor(message.member.displayHexColor).setAuthor({name: message.member.displayName, iconURL: author.avatarURL({ dynamic: true, size: 2048 })}).setFooter({text: (config.bot.BotFooter) , iconURL: kemosalvo.avatarURL({ dynamic: true, size: 2048 })})
    if (!names) {
